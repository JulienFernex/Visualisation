...
def create_population_distribution_histogram(selected_col=COL_POPULATION):
    """
    Crée un histogramme de distribution montrant le nombre de départements 
    pour chaque plage de la métrique sélectionnée
    """
    try:
        df = pd.read_csv(CLEAN_DATA_PATH)
    except FileNotFoundError:
    ...
    # Définir le titre et le label en fonction de la métrique
    if selected_col == COL_POPULATION:
        titre = "Distribution de la Population par Département"
        range_label = "Plage de Population"
    elif selected_col == COL_RATIO:
        titre = "Distribution de la Densité (pour 100k hab) par Département"
        range_label = "Plage de Densité"
    else:
        titre = "Distribution du Nombre d'Établissements par Département"
        range_label = "Plage d'Établissements"
    
    # Créer des bins (plages) de la métrique sélectionnée
    # Utiliser 10 bins de taille égale
    n_bins = 10
    metric_min = df[selected_col].min()
    metric_max = df[selected_col].max()
    
    # Créer les bins avec une marge relative pour inclure la valeur max
    margin = (metric_max - metric_min) * 0.01  # 1% du range
    bins = np.linspace(metric_min, metric_max + margin, n_bins + 1)
    
    # Assigner chaque département à un bin
    df['Value_Range'] = pd.cut(df[selected_col], bins=bins, labels=False, include_lowest=True)
    
    # Compter le nombre de départements par range
    dist_data = df.groupby('Value_Range').size().reset_index(name='Nombre_Departements')
    
    # Créer des labels plus lisibles pour les ranges
    range_labels = []
    ...
    dist_data['Range_Label'] = range_labels[:len(dist_data)]
    
    # Créer le graphique
    fig = px.bar(
        dist_data,
        x='Range_Label',
        y='Nombre_Departements',
        color='Nombre_Departements',
    ...
    fig.update_yaxes(title_text="Nombre de Départements", showgrid=True, gridcolor='#eee')
    
    return fig


def create_population_distribution_pie(selected_col=COL_POPULATION):
    """
    Crée un camembert de distribution montrant la proportion de départements 
    pour chaque plage de la métrique sélectionnée
    """
    try:
        df = pd.read_csv(CLEAN_DATA_PATH)
    ...
    # Définir le titre et le label en fonction de la métrique
    if selected_col == COL_POPULATION:
        titre = "Part de Départements par Plage de Population"
        range_label = "Plage de Population"
    elif selected_col == COL_RATIO:
        titre = "Part de Départements par Plage de Densité"
        range_label = "Plage de Densité"
    else:
        titre = "Part de Départements par Plage d'Établissements"
        range_label = "Plage d'Établissements"
