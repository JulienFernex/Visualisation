...
from src.components.bubble_chart import create_bubble_chart
from src.components.bar_chart import create_bar_chart
from src.components.dist_hist import create_dist_hist
from src.components.pie_chart import create_pie_chart
    ...
    html.Div([
        ...
        html.Div([
            dcc.Loading(
                ...
                children=dcc.Graph(
                    id='histogram-chart',
                    figure=create_bar_chart(COL_VALUE),
                    style={'height': '600px'}
                ),
                style={'height': '600px'}
            )
...
def update_dashboard(selected_metric, selected_department):
    ...
    hist_fig = create_bar_chart(selected_metric, department=dept)
    dist_hist_fig = create_dist_hist(selected_metric, department=dept)
    dist_pie_fig = create_pie_chart(selected_metric, department=dept)
